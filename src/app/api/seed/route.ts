...
const SEED_SECRET = process.env.SEED_SECRET || 'vocabmaster-seed-2024';
...
const templates = [
    ...
    },
    {
        ...
        words: [
            ...
            { word: 'schedule', turkishTranslation: 'program, takvim', definitionTr: 'Planlı zaman çizelgesi', exampleSentence: 'What is your schedule like this week?', exampleSentenceTr: 'Bu hafta programın nasıl?', type: 'noun', level: 'A2' },
    ...
    },
    {
        ...
        words: [
            ...
            { word: 'itinerary', turkishTranslation: 'seyahat planı', definitionTr: 'Gezi rotası ve programı', exampleSentence: 'What is our itinerary for tomorrow?', exampleSentenceTr: 'Yarın için seyahat planımız ne?', type: 'noun', level: 'B2' },
            ...
            { word: 'aisle', turkishTranslation: 'koridor', definitionTr: 'İki sıra arasındaki geçit', exampleSentence: 'I would like an aisle seat, please.', exampleSentenceTr: 'Koridor tarafı koltuk istiyorum, lütfen.', type: 'noun', level: 'B1' },
...
// GET - Trigger seeding (requires secret)
        ...
        for (const template of templates) {
            ...
            for (const wordData of template.words) {
                let word = await prisma.word.findFirst({
                    where: { word: wordData.word, level: wordData.level }
                });
            ...
            await prisma.wordList.create({
                data: {
                    ...
                    isSystem: true,
                    items: {
                        create: createdWords.map(w => ({ wordId: w.id }))
                    }
                }
            });

            results.push({ name: template.name, status: 'created', wordCount: createdWords.length });
        }
